...
def index(request):
    ...
    feed_url = request.build_absolute_uri(reverse('dropfeed:feed', kwargs={'path': feed_path}))
...
def feed(request, path):
    ...
    feed_data = {
        ...
        'itunes_category': config.DROPFEED_ITUNES_CATEGORY,
    ...
    xml_content = render_to_string('dropfeed/feed.xml', {'feed': feed_data}, request=request)
